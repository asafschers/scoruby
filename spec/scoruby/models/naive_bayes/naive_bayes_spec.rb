...
describe Scoruby::Models::NaiveBayes::Model do
  ...
  let(:l1) { 2557 * (Math.exp(-(24 - 24.936)**2 / (2 * 0.516) ) / Math.sqrt(Math::PI*2 * 0.516)) * 1321/2533 * 10/2436 * 182/2266 }
  let(:l2) { 1530 * (Math.exp(-(24 - 24.588)**2 / (2 * 0.635) ) / Math.sqrt(Math::PI*2 * 0.635)) * 780/1522 * 9/1496 * 51/1191 }
  let(:l3) { 709 * (Math.exp(-(24 - 24.428)**2 / (2 * 0.379) ) / Math.sqrt(Math::PI*2 * 0.379)) * 405/697 * 0.001 * 26/699 }
  let(:l4) { 100 * (Math.exp(-(24 - 24.770)**2 / (2 * 0.314) ) / Math.sqrt(Math::PI*2 * 0.314)) * 42/90 * 10/98 * 6/87 }
